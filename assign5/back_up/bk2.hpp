...
// under construction
        ...
        */
        ...
        /*
        friend std::ostream & operator << (std::ostream &out, const Matrix<complex<double>,M,N> &obj) {
            for (int i = 0; i < M; i++){
                out << "| " ;
                for (int j = 0; j < N; j++){
                    out.width(4);
...
void Matrix< T, M, N >::Print() {
    if (typeid(T).name() == typeid(complex<double>).name()){
        for (int i = 0; i < M; i++){
            cout << "| " ;
            for (int j = 0; j < N; j++){
                cout.width(4);
                cout.precision(4);
                if ((this->Vec[ i*N + j]).imag() > 0)
                cout << fixed << right << (this->Vec [i*N + j]).real() << " + " << (this->Vec[ i*N + j]).imag() << "i  " ;
                else
                cout << fixed << right << (this->Vec [i*N + j]).real() << " - " << abs( (this->Vec[ i*N + j]).imag() )<< "i  " ;
            }
            cout << "|" <<  endl;
        }
    }else {
...
void Matrix< complex<double>,M,N >::Print() {
    for (int i = 0; i < M; i++){
        cout << "| " ;
        for (int j = 0; j < N; j++){
            cout << fixed << scientific << right << (this->Vec [i*N + j]).real() << " + " << (this->Vec[ i*N + j]).imag() << "i  " ;
        }
        cout << "|" <<  endl;
    }
};
